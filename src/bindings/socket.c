...
/*=============================================================================
 ...
 ============================================================================*/
#ifndef _WIN32
	COMO_METHOD(como_sock_sendfd) {
		int socket  = duk_require_int(ctx, 0);
		int fd      = duk_require_int(ctx, 1);

		void *buf = duk_push_fixed_buffer(ctx, CMSG_SPACE (sizeof (int)));
	...
	#define UV__CMSG_FD_COUNT 1
	#define UV__CMSG_FD_SIZE (UV__CMSG_FD_COUNT * sizeof(int))

	COMO_METHOD(como_sock_recvfd) {
		int socket  = duk_require_int(ctx, 0);

		// void *buf = duk_push_fixed_buffer(ctx, CMSG_SPACE (sizeof (int)));
		size_t buflen;
		void *buf = duk_require_buffer_data(ctx, 1, &buflen);
		int *passfd = duk_require_buffer_data(ctx, 2, NULL);
...
#else
	COMO_METHOD(como_sock_sendfd) {
		duk_error(ctx, DUK_ERR_TYPE_ERROR, "sock.sendfd not supported");
		return 1;
	}
	COMO_METHOD(como_sock_recvfd) {
		duk_error(ctx, DUK_ERR_TYPE_ERROR, "sock.recvfd not supported");
		return 1;
	}
#endif
