...
COMO_METHOD(como_http_parser_init) {
	enum http_parser_type type = duk_get_int(ctx, 0);
	void *self = duk_get_heapptr(ctx, 1);

	http_parser *p = malloc(sizeof(*p));
	if (p == NULL) COMO_SET_ERRNO_AND_RETURN(ctx, ENOMEM);
...
COMO_METHOD(como_http_parser_execute) {
...
COMO_METHOD(como_http_status_code) {
	http_parser *p  = duk_require_pointer(ctx, 0);
	int ret = p->status_code;
	duk_push_int(ctx, ret);
	return 1;
}
...
COMO_METHOD(como_http_parser_methods) {
	duk_idx_t arr_idx = duk_push_array(ctx);
	duk_push_string(ctx, "http_get");
	duk_put_prop_index(ctx, arr_idx, 0);
	#define V(num, name, string) \
		duk_push_string(ctx, #string); \
		duk_put_prop_index(ctx, arr_idx, num);
		HTTP_METHOD_MAP(V)
	#undef V
	return 1;
}
...
static const duk_function_list_entry como_http_parser_funcs[] = {
	...
	{ "execute"                 , como_http_parser_execute, 3 },
	...
	{ "status_code"             , como_http_status_code, 1 },
	{ "destroy"                 , como_http_parser_destroy, 1 },
	{ "methods"                 , como_http_parser_methods, 0},
	{ NULL                      , NULL, 0 }
};


static int init_binding_httpParser(duk_context *ctx) {
	duk_push_object(ctx);
