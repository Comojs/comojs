...
#include <fcntl.h>
	...
	size_t pread (int fd, void *buffer, size_t len, off_t position){
		// posix pread doesn't affect current file position
		// we need to get current file position and then
		// reset it back to that position
...
COMO_METHOD(como_posix_read) {
	...
	if (duk_is_number(ctx, 1)){
		...
		if (duk_is_number(ctx, 3)){
			isPread = 1;
			position = (off_t)duk_get_uint(ctx, 3);
		}
...
int como__readdir(duk_context *ctx, const char *dir, size_t length_of_dir) {
	...
	int index = 0;
		...
		char *newdir = duk_push_fixed_buffer(ctx, length_of_dir + 2);
		memcpy(newdir, dir, length_of_dir + 3);

		newdir[length_of_dir] = '\\';
		newdir[length_of_dir + 1] = '*';
		newdir[length_of_dir + 2] = '\0';
